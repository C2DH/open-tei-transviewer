...
BookReader.prototype.init = function() {
  // // console.log("(1) initiate BookReader");  
    ...
    $(".doc-part-fac").append("<div id='BRcontainer' dir='ltr'></div>");
   ...
   // console.log("viewWidth: "+viewWidth);
    ...
    for (i=0; i<indicesToDisplay.length; i++) {
        ...
        if (BookReader.util.notInArray(indicesToDisplay[i], this.displayedIndices)) {
            ...
            //div.style.width = $("#BRpageview img").width()+'px';
            //div.style.height= $("#BRpageview img").height()+'px';
            
            $(div).css('top', leafTop + 'px');
            var left = (viewWidth-width)>>1;
            if (left<0) left = 0;
           // $(div).animate({left: left+'px'},300);
            ...
            //$(div).animate({width: width+"px", height: height+"px"}, 'slow');

            // if($(".BRpagediv1up").length < 1){
                $('#BRpageview').append(div).show('slow');
            // }
                
            var globalWindow = $(document).width();
            var scanWidth = $("#BRcontainer").width();
            var transWidth = $(".text-part").width();
            var div1up = $(".BRpagediv1up").width();
            //console.log("scanWidth: "+scanWidth);
            //console.log("div1up: "+div1up);
            var newLeft = (scanWidth - div1up) / 2;
            //console.log("newLeft: "+newLeft);

            //console.log("global window: "+globalWindow+"\nscan part: "+scanWidth+"\ntrans part: "+transWidth);
            if(globalWindow < 1000){
                
                $("#pagediv0").css('left', newLeft +'px');
               ...
               // console.log("/!§");
               // console.log("/! this._getPageURI("+index+", "+this.reduce+", 0);");
                $(img).addClass('BRnoselect magniflier');
                

                $(img).css('width', width+"px").css('height', height+"px");
                
                //  f i x
                //$(img).css('width', $("#BRpageview img").width()+'px').css('height', $("#BRpageview img").height()+'px');
                //$(img).animate({width: width+"px", height: height+"px"}, 300);
                $(div).append(img);

                // $(img).css({
                //   '-webkit-transform' : 'scale(' + height+'px',
                //   '-moz-transform'    : 'scale(' + height+'px',
                //   '-ms-transform'     : 'scale(' + height+'px',
                //   '-o-transform'      : 'scale(' + height+'px',
...
//console.log("resize page view here");
...
BookReader.prototype.resizePageView1up = function() {
    ...
    //var newLeft = 700;
    var newLeft = newCenterX - $('#BRcontainer').prop('clientWidth');
    newLeft = Math.max(newLeft, 0);
    setTimeout(function() {
//        console.log('newLeft: ' + newLeft);
        $('#BRcontainer').prop('scrollLeft', $('#BRcontainer').prop('clientWidth') / 1.20);
    });
...
BookReader.prototype.prepareOnePageView = function() {
    //console.log("(5) prepareOnePageView");
...
BookReader.prototype.prepareTwoPagePopUp = function() {
    ...
    $(this.leafEdgeL).bind('mousemove', this, function(e) {
        ...
        //           (https://bugs.edge.launchpad.net/gnubook/+bug/327456)
        $(e.data.twoPagePopUp).css({
            left: e.pageX - $('#BRcontainer').offset().left + $('#BRcontainer'). Left() - $(e.data.twoPagePopUp).width() + 100 + 'px',
            top: e.pageY-$('#BRcontainer').offset().top + $('#BRcontainer').scrollTop() + 'px'
        });
    });
}
...
/* 2016/12/09 - fre - Jira WEBSIX-383 ([TEI Transviewer][Fit to page] The scans are not always completely visible after loading. Adjust "fit to page".) */
/* Add auto fit mode for one page view */
BookReader.prototype.onePageGetAutofitAuto = function() {
	
    var widthPadding 	= 20;
    var imageRatio		= this.getMedianPageSize().width / this.getMedianPageSize().height;
    var brRatio			= ($('#BRcontainer').prop('clientWidth') - widthPadding * 2) / ($('#BRcontainer').prop('clientHeight') - this.padding * 2);
    
	if(imageRatio > brRatio)
		return this.onePageGetAutofitWidth();
	else
		return this.onePageGetAutofitHeight();
}
...
BookReader.prototype.onePageCalculateReductionFactors = function( width, height ) {
    this.onePage.reductionFactors = this.reductionFactors.concat(
        [
            ...
            /* 2016/12/09 - fre - Jira WEBSIX-383 ([TEI Transviewer][Fit to page] The scans are not always completely visible after loading. Adjust "fit to page".) */
            /* Add auto fit mode for one page view */
            { reduce: this.onePageGetAutofitAuto(), autofit: 'auto'},
        ]);
...
BookReader.prototype.showEmbedCode = function() {
    ...
    $('.doc-part-fac').append('<div class="coverUp" style="position:absolute;z-index:299;width:100%;height:100%;background:#000;opacity:.4;filter:alpha(opacity=40);" onclick="$(\'.popped\').hide();$(this).hide();"></div>');
...
BookReader.prototype.initToolbar = function(mode, ui) {
   //// console.log("(2) init toolbar");
    ...
    if (ui == "embed" || ui == "touch") {
        $(".doc-part-fac a.logo").attr("target","_blank");
    }
...
BookReader.prototype.bindNavigationHandlers = function() {
    //console.log("(6) bindNavigationHandlers");
    ...
    this.initSwipeData();
    $('.doc-part-fac').off('mousemove.navigation').on('mousemove.navigation',
        { 'br': this },
...
BookReader.prototype.unbindNavigationHandlers = function() {
    $('.doc-part-fac').die('mousemove.navigation');
}

// navigationMousemoveHandler
//______________________________________________________________________________
// Handle mousemove related to navigation.  Bind at .doc-part-fac level to allow autohide.
...
BookReader.prototype.swipeMousemoveHandler = function(event) {
    ...
    var minSwipeLength = Math.min($('.doc-part-fac').width() / 5, 80);
...
BookReader.prototype.bindMozTouchHandlers = function() {
    ...
    $('.doc-part-fac').bind('MozTouchDown', function(event) {
...
BookReader.prototype.updateTheme = function(theme) {
    ...
    if (theme == this.default_theme) {
        $('#BRCSSTheme').prop('disabled', true);
        return;
    }

    var stylesheet = $('#BRCSSTheme');
    if (stylesheet.length == 0) {
        stylesheet = $('<link rel="stylesheet" type="text/css">').prop('id', 'BRCSSTheme');
        $('head').append(stylesheet);
    }

    var main_href = $('#BRCSS').prop('href');
...
BookReader.prototype.updateFromParams = function(params) {
   // console.log("(6) updateFromParams");
    if ('undefined' != typeof(params.mode)) {
...
BookReader.prototype.updateLocationHash = function() {
   // var newHash = '#/scan/' + this.fragmentFromParams(this.paramsFromCurrent());
   //// console.log(newHash);
   // return false;   
...
BookReader.prototype.startLocationPolling = function() {
    ...
    // //console.log("(3) start location polling");
...
BookReader.prototype.showProgressPopup = function(msg) {
    ...
    $(this.popup).css({
        top:      ($('.doc-part-fac').height()*0.5-100) + 'px',
        left:     ($('.doc-part-fac').width()-300)*0.5 + 'px'
    ...
    $(this.popup).appendTo('.doc-part-fac');
}
...
BookReader.prototype.initUIStrings = function()
{
    ...
    for (var icon in titles) {
        if (titles.hasOwnProperty(icon)) {
            $('.doc-part-fac').find(icon).prop('title', titles[icon]);
        }
    }
